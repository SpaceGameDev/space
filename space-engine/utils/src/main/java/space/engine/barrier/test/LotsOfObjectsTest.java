...
import space.engine.barrier.test.TransactionTest.Entity;
...
public class LotsOfObjectsTest {
	...
	private static Result run(int objectsCount) throws InterruptedException {
		...
		Barrier task = when(IntStream.range(0, CORES)
									 .mapToObj(i -> start.thenRun(() -> {
										 List<Barrier> transactions = IntStream.range(partSize * i, partSize * (i + 1) + (i == CORES - 1 ? partRest : 0))
																			   .boxed()
																			   .flatMap(x -> IntStream.range(0, world.length)
																									  .mapToObj(y -> new IntVector(x, y))
																									  .filter(v -> v.x != v.y))
																			   .map(v -> TransactionTest.transaction(world[v.x], world[v.y], 1))
																			   .collect(Collectors.toUnmodifiableList());
										 transactionCount.addAndGet(transactions.size());
										 throw new DelayTask(when(transactions));
									 }))
		);
		if (TIMER_PRINTOUT)
