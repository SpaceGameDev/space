	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		try (Frame frame = allocatorStack().frame()) {
			...
			//vulkan
			{
				...
				//swapchain
				VkRect2D swapExtend = mallocStruct(frame, VkRect2D::create, VkRect2D.SIZEOF).set(
				...
				//renderPass
				VkRenderPass renderPass = VkRenderPass.alloc(mallocStruct(frame, VkRenderPassCreateInfo::create, VkRenderPassCreateInfo.SIZEOF).set(
						...
						null
				), device, new Object[] {frame});
				
				//pipeline layout
				VkPipelineLayout pipelineLayout = VkPipelineLayout.alloc(mallocStruct(frame, VkPipelineLayoutCreateInfo::create, VkPipelineLayoutCreateInfo.SIZEOF).set(
				...
				//shader module
				VkShaderModule shaderModuleVert = VkShaderModule.alloc(device, Objects.requireNonNull(FirstTriangle.class.getResourceAsStream("firstTriangle.vert.spv")).readAllBytes(), new Object[] {frame});
				VkShaderModule shaderModuleFrag = VkShaderModule.alloc(device, Objects.requireNonNull(FirstTriangle.class.getResourceAsStream("firstTriangle.frag.spv")).readAllBytes(), new Object[] {frame});
				
				//pipeline
				VkGraphicsPipeline pipeline = VkGraphicsPipeline.alloc(mallocStruct(frame, VkGraphicsPipelineCreateInfo::create, VkGraphicsPipelineCreateInfo.SIZEOF).set(
				...
				VkFramebuffer[] framebuffers = Arrays
						...
						.map(swapChainImageView -> VkFramebuffer.alloc(mallocStruct(frame, VkFramebufferCreateInfo::create, VkFramebufferCreateInfo.SIZEOF).set(
				...
				VkCommandPool commandPool = VkCommandPool.alloc(mallocStruct(frame, VkCommandPoolCreateInfo::create, VkCommandPoolCreateInfo.SIZEOF).set(
				...
				VkCommandBuffer[] commandBuffers = commandPool.allocCmdBuffers(VK_COMMAND_BUFFER_LEVEL_PRIMARY, framebuffers.length, new Object[] {frame});
				...
				VkSemaphore semaphoreImageAvailable = VkSemaphore.alloc(mallocStruct(frame, VkSemaphoreCreateInfo::create, VkSemaphoreCreateInfo.SIZEOF).set(
						VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO,
						0,
						0
				), device, new Object[] {frame});
				VkSemaphore semaphoreRenderFinished = VkSemaphore.alloc(mallocStruct(frame, VkSemaphoreCreateInfo::create, VkSemaphoreCreateInfo.SIZEOF).set(
						VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO,
						0,
						0
				), device, new Object[] {frame});
